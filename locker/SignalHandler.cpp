...
#ifdef _WIN32
	switch(WIN32_physicalToLogical(signal)) {
#else
	switch(POSIX_physicalToLogical(signal)) {
#endif
		case SignalHandler::SIG_RELOAD: Q_EMIT g_handler->s_reload(); break;
		case SignalHandler::SIG_INT: Q_EMIT g_handler->s_int(); break;
		case SignalHandler::SIG_CLOSE: Q_EMIT g_handler->s_close(); break;
		case SignalHandler::SIG_TERM: Q_EMIT g_handler->s_term(); break;
		default: break;
...
DWORD WIN32_logicalToPhysical(int signal) {
    switch (signal) {
		...
		default:  return ~(unsigned int)0; // SIG_ERR = -1
    }
}
#else
...
int WIN32_physicalToLogical(DWORD signal) {
    switch (signal) {
		...
		default: return SignalHandler::SIG_UNHANDLED;
    }
}
#else
